...
// 계산된 값: computed()
// 숙박 일수를 자동으로 계산하는 computed를 만든다.
const nights = computed(() => {
  // 만약 체크인/체크아웃 날짜가 없으면 => 숙박일은 0 : return
  ...
  //   (1000 * 60 * 60 * 24) => 밀리초 단위를 하루 단위로 변환
  const diff = (outDate - inDate) / (1000 * 60 * 60 * 24);
  return diff > 0 ? diff : 0;
});
// console.log(nights.value);

// 1박(하루) 1인당 숙박요금 : 50,000원
const pricePerNight = 50000;
// 총금액을 자동으로 계산하는 computed
const totalPrice = computed(() => {
  // {} 괄호가 없으면 return이 없어도 된다.
