<template>
  <div>
    ...
    <ul>
      <!-- v-for = "변수 in 배열" -->
      <!-- <li v-for="fruit in fruits" :key="index">{{ fruit }}</li> -->
    ...
    <h2>글 목록</h2>
    <ul>
      <li v-for="(item, index) in posts" :key="index">{{ item }}</li>
    </ul>
    <h2>메뉴 목록</h2>
    <ul>
      <li v-for="(item, index) in menus" :key="index">{{ index + 1 }} . {{ item }}</li>
    </ul>
  </div>
</template>
...
const products = ref(["피자", "햄버거", "치킨", "콜라"]);
// 글 배열
