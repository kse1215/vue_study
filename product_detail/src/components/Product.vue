<template>
  <div class="home">
    ...
    <ul class="product_list">
      <li v-for="(pro, index) in products" :key="index" @click="viewProductDetails(pro)" class="card">
        <h3>{{ pro.name }}</h3>
        <img :src="pro.image" :alt="pro.name" />
        <p>{{ pro.description }}</p>
        <p>{{ pro.price }}</p>
        <button class="button primary">자세히 보기</button>
      </li>
    </ul>
  </div>
  ...
  <div v-if="showProductModal" class="modal_overlay">
    <div class="modal">
      ...
      <button class="button secondary" @click="showProductModal = false">닫기</button>
    </div>
  </div>
</template>
